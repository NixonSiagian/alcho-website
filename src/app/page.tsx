...
/* ═══════════════════════════════════════════════════════════════
   ...
   Structure: Hero → Brand Story → Products → Highlight → CTA
   
   Design principles:
   • Clean visual hierarchy with generous whitespace
   • Parallax ONLY in Hero + Highlight section
   • Subtle scroll reveals (fade + translate only)
   • Gold gradient accents, premium serif typography
   • Mobile: no parallax, simple fades
   ═══════════════════════════════════════════════════════════════ */
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      ════════════════════════════════════════ */}
      <section className="py-24 md:py-32 bg-cream-50 dark:bg-brown-950">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          ...
          <StaggerChildren className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f, i) => (
              <motion.div key={f.title} variants={staggerItem}>
                <div className="h-full bg-cream-50 dark:bg-brown-900/50 rounded-2xl p-7 border border-brown-100/60 dark:border-brown-800 transition-all duration-400 ease-luxury hover:border-gold-300/50 hover:-translate-y-1">
                  <span className="inline-flex items-center justify-center w-11 h-11 rounded-full bg-gold-100 dark:bg-gold-900/30 text-gold-600 dark:text-gold-300 mb-5">
                    <f.icon className="w-5 h-5" />
                  </span>
                  <h3 className="font-serif text-lg font-semibold text-brown-900 dark:text-cream-100 mb-2">
                    {f.title}
                  </h3>
          ...
          <div className="mt-20 lg:mt-28 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <ScrollReveal direction="up" distance={24} delay={0.15}>
              <span className="text-gold-600 dark:text-gold-400 text-[11px] font-semibold tracking-[0.3em] uppercase">
                Our Heritage
              </span>
              <h3 className="font-serif text-3xl md:text-4xl font-semibold text-brown-900 dark:text-cream-50 mt-4 leading-[1.2]">
                From the spice islands to{" "}
                <span className="text-gold-gradient">your kitchen.</span>
              </h3>
              <p className="mt-5 text-brown-600 dark:text-brown-300 leading-relaxed">
      ...
      {/* ════════════════════════════════════════
          3. PRODUCTS — Featured collection
      ════════════════════════════════════════ */}
      <section className="py-24 md:py-32 bg-cream-100/50 dark:bg-brown-900/30">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-6 mb-14">
            <ScrollReveal direction="up" distance={20} className="max-w-xl">
              ...
              <h2 className="font-serif text-3xl md:text-4xl lg:text-[42px] font-semibold leading-[1.15] text-brown-900 dark:text-cream-50 mt-4">
                Handcrafted seasonings, born of{" "}
                <em className="italic text-gold-700 dark:text-gold-300 not-italic font-serif">Nusantara.</em>
              </h2>
            </ScrollReveal>
            ...
            <FadeIn delay={0.2}>
              <Link
                href="/products"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-brown-200 dark:border-brown-700 text-brown-800 dark:text-cream-100 text-sm font-medium hover:border-gold-400/50 transition-all duration-300"
              >
                View All
      ...
      {/* ════════════════════════════════════════
          4. HIGHLIGHT — Cinematic parallax section (second parallax usage)
      ════════════════════════════════════════ */}
      <section className="relative h-[70vh] min-h-[500px] max-h-[700px] flex items-center overflow-hidden">
        <ParallaxImage strength={60} className="absolute inset-0">
          <Image
        ...
        {/* Overlays */}
        <div className="absolute inset-0 bg-brown-950/70" />
        <div className="absolute inset-0 bg-gradient-to-r from-brown-950/40 to-transparent" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full px-5 sm:px-8 lg:px-12">
          <ScrollReveal direction="up" distance={24} className="max-w-2xl">
            <span className="text-gold-300 font-serif italic text-lg block mb-4">
      ...
      {/* ════════════════════════════════════════
          5. CTA — Final conversion moment
      ════════════════════════════════════════ */}
      <section className="py-24 md:py-32 bg-brown-950">
